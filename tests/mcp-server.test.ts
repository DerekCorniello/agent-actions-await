...
function makeWatchHelpers(sha = "sha-mcp", status: "in_progress" | "completed" = "in_progress") {
  ...
  const getChecks = vi.fn(async () => [{ name: "ci", status, conclusion: status === "completed" ? "success" : null, sha }]);
...
describe("mcp-server", () => {
  it("lists three tools", async () => {
    ...
    expect(res.tools.map((t) => t.name).sort()).toEqual(["await_pr_actions", "get_pr_watch_status", "start_pr_watch"]);
  });
  ...
  it("start_pr_watch returns handle and get_pr_watch_status reflects pending/completed", async () => {
    ...
    const start = await client.callTool({ name: "start_pr_watch", arguments: { owner: "acme", repo: "demo", pr_number: 1 } });
    ...
    const pending = await client.callTool({ name: "get_pr_watch_status", arguments: { handle } });
    const ptxt = (pending.content as Array<{ text: string }>)[0]!.text;
    expect(JSON.parse(ptxt).state).toBe("pending");

    bus.publish({ owner: "acme", repo: "demo", sha: "sha1", type: "check_run", status: "completed", conclusion: "success", name: "ci" });
  ...
  it("get_pr_watch_status returns handle_not_found after first read of completed (Q37)", async () => {
    ...
    const wm = new WatchManager(bus, { getPrHeadShaFn: getPrHeadSha as unknown as typeof getPrHeadSha, getChecksFn: getChecks as unknown as typeof getChecks });
    const { client } = await linkedClient(bus, wm);
    const start = await client.callTool({ name: "start_pr_watch", arguments: { owner: "acme", repo: "demo", pr_number: 2 } });
    const handle = (JSON.parse((start.content as Array<{ text: string }>)[0]!.text) as { handle: string }).handle;
    await client.callTool({ name: "get_pr_watch_status", arguments: { handle } });
    const second = await client.callTool({ name: "get_pr_watch_status", arguments: { handle } });
  ...
  it("invalid arguments return isError with zod message", async () => {
    ...
    const wm = new WatchManager(bus, { getPrHeadShaFn: getPrHeadSha as unknown as typeof getPrHeadSha, getChecksFn: getChecks as unknown as typeof getChecks });
    const { client } = await linkedClient(bus, wm);
    const res = await client.callTool({ name: "start_pr_watch", arguments: { owner: "", repo: "demo", pr_number: -1 } });
  ...
  it("unknown tool returns isError", async () => {
    const bus = new EventBus();
    const fakeHead = vi.fn(async () => "s") as unknown as typeof import("../src/github.js").getPrHeadSha;
    const fakeChecks = vi.fn(async () => []) as unknown as typeof import("../src/github.js").getChecksForShaWithGrace;
  ...
  it("await_pr_actions blocks until webhook completes and sends progress", async () => {
    ...
    const wm = new WatchManager(bus, { getPrHeadShaFn: getPrHeadSha as unknown as typeof getPrHeadSha, getChecksFn: getChecks as unknown as typeof getChecks }, { defaultTimeoutMs: 5000 });
    const { client } = await linkedClient(bus, wm);
    const p = client.callTool({ name: "await_pr_actions", arguments: { owner: "acme", repo: "demo", pr_number: 3, timeout_seconds: 2 } });
    setTimeout(() => bus.publish({ owner: "acme", repo: "demo", sha: "sha-await", type: "check_run", status: "completed", conclusion: "success", name: "ci" }), 20);
    const res = await p;
    const txt = (res.content as Array<{ text: string }>)[0]!.text;
  ...
  it("await_pr_actions times out to timed_out", async () => {
    ...
    const { client } = await linkedClient(bus, wm);
    const res = await client.callTool({ name: "await_pr_actions", arguments: { owner: "acme", repo: "demo", pr_number: 4, timeout_seconds: 1 } });
    const txt = (res.content as Array<{ text: string }>)[0]!.text;
  ...
  it("filter exact match: only wanted check completes", async () => {
    ...
    const wm = new WatchManager(bus, { getPrHeadShaFn: getPrHeadSha as unknown as typeof getPrHeadSha, getChecksFn: getChecks as unknown as typeof getChecks });
    const { client } = await linkedClient(bus, wm);
    const start = await client.callTool({ name: "start_pr_watch", arguments: { owner: "acme", repo: "demo", pr_number: 5, filter: "ci" } });
    const handle = (JSON.parse((start.content as Array<{ text: string }>)[0]!.text) as { handle: string }).handle;
    bus.publish({ owner: "acme", repo: "demo", sha: "sha-f", type: "check_run", status: "completed", conclusion: "success", name: "lint" });
    await new Promise((r) => setTimeout(r, 15));
    let txt1 = (await client.callTool({ name: "get_pr_watch_status", arguments: { handle } }).then((r) => (r.content as Array<{ text: string }>)[0]!.text)) as string;
    let st = (JSON.parse(txt1) as { state: string }).state;
    expect(st).toBe("pending");
    bus.publish({ owner: "acme", repo: "demo", sha: "sha-f", type: "check_run", status: "completed", conclusion: "success", name: "ci" });
    await new Promise((r) => setTimeout(r, 15));
    const txt2 = (await client.callTool({ name: "get_pr_watch_status", arguments: { handle } }).then((r) => (r.content as Array<{ text: string }>)[0]!.text)) as string;
