...
import { downloadUrl, ensureCloudflared, startTunnel } from "../src/tunnel.js";
...
describe("downloadUrl", () => {
    ...
    it("falls back to linux-amd64 for unknown platform", () => {
        expect(downloadUrl("2024.11.1", "freebsd" as NodeJS.Platform, "mips")).toContain(
...
describe("ensureCloudflared", () => {
    ...
    it("downloads and verifies checksum when not in PATH", async () => {
        ...
        const maybe = await ensureCloudflared({
            expectedSha256: hash,
            cacheDir: tmp,
            deps: { fetchFn: fetchFn as unknown as typeof fetch, cacheDirFn: () => tmp },
        }).catch(() => null);
...
describe("startTunnel", () => {
    it("parses trycloudflare URL from stdout/stderr", async () => {
        ...
        setTimeout(
            () =>
                stderr.write(
                    "2024/11/01 00:00:00 INF |  https://random123.trycloudflare.com                                                       |\n",
                ),
            10,
        );
        const t = await p;
    ...
    it("rejects if process exits before URL", async () => {
        ...
        setTimeout(
            () => (ee as unknown as { emit: (a: string, b: unknown) => void }).emit("exit", 1),
            10,
        );
