...
describe("TunnelManager", () => {
  ...
  it("start calls ensureBin/start, emits url and repatches hooks", async () => {
    const fakeProc = { once: vi.fn(), on: vi.fn(), kill: vi.fn() } as unknown as import("node:child_process").ChildProcess;
    const fakeTunnel = { url: "https://xyz.trycloudflare.com", proc: fakeProc, stop: vi.fn() } as unknown as import("../src/tunnel.js").Tunnel;
  ...
  it("uses binPath directly when provided, skipping ensureBin", async () => {
    const fakeProc = { once: vi.fn(), on: vi.fn(), kill: vi.fn() } as unknown as import("node:child_process").ChildProcess;
    const fakeTunnel = { url: "https://abc.trycloudflare.com", proc: fakeProc, stop: vi.fn() } as unknown as import("../src/tunnel.js").Tunnel;
    const ensureBin = vi.fn(async () => "/should/not/be/called");
    const start = vi.fn(async () => fakeTunnel);
    const mgr = new TunnelManager({ port: 4000, binPath: "/tmp/from-arg", ensureBin, start: start as unknown as typeof import("../src/tunnel.js").startTunnel, backoffMs: 10 });
    await mgr.start();
