...
import { configDir, configPath, secretsDir, secretPathFor, loadConfig, saveConfig, ensureSecret, readSecret, makeSecretGetter } from "../src/config.js";
...
describe("config", () => {
  it("configDir respects XDG_CONFIG_HOME", () => {
    expect(configDir({ XDG_CONFIG_HOME: "/tmp/x" } as NodeJS.ProcessEnv)).toBe("/tmp/x/agent-actions-await");
    expect(configPath({ XDG_CONFIG_HOME: "/tmp/x" } as NodeJS.ProcessEnv)).toBe("/tmp/x/agent-actions-await/config.json");
    expect(secretsDir({ XDG_CONFIG_HOME: "/tmp/x" } as NodeJS.ProcessEnv)).toBe("/tmp/x/agent-actions-await/secrets");
    expect(secretPathFor("o", "r", { XDG_CONFIG_HOME: "/tmp/x" } as NodeJS.ProcessEnv)).toBe("/tmp/x/agent-actions-await/secrets/o__r.txt");
  });
  ...
  it("save/load round-trip", async () => {
    const { env, cleanup } = tempEnv();
    await saveConfig({ repos: [{ owner: "acme", repo: "demo", secretPath: "/tmp/s" }], port: 0, pollGraceSeconds: 30 }, env);
