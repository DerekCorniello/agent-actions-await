...
export async function isInPath(bin = "cloudflared", deps?: TunnelDeps): Promise<string | null> {
...
export async function ensureCloudflared(opts: {
  ...
  if (existsSync(binPath)) {
    ...
    if (opts.expectedSha256) {
      ...
      if (hash !== opts.expectedSha256) {
        throw new Error(`cloudflared checksum mismatch expected ${opts.expectedSha256} got ${hash}`);
      }
    }
    return binPath;
  }
  ...
  // Download
  const url = downloadUrl(opts.version ?? CLOUDFLARED_VERSION, deps.platformFn?.() ?? platform(), deps.archFn?.() ?? arch());
  ...
  if (opts.expectedSha256) {
    ...
    if (hash !== opts.expectedSha256) throw new Error(`downloaded cloudflared checksum mismatch expected ${opts.expectedSha256} got ${hash}`);
  }
 ...
 */
export async function startTunnel(port: number, binPath: string, deps?: TunnelDeps): Promise<Tunnel> {
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    const onExit = (code: number | null) => {
      cleanup();
      reject(new Error(`cloudflared exited ${code} before URL appeared: ${out.slice(0, 500)}`));
    };
    const cleanup = () => {
