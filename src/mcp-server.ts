...
const StartWatchInput = z.object({
  ...
  filter: z.union([z.literal("all"), z.string().min(1), z.array(z.string().min(1)).min(1)]).optional(),
...
export function createMcpServer(opts?: { bus?: EventBus; watchManager?: WatchManager }): Server {
  ...
  const server = new Server({ name: "agent-actions-await", version: "0.1.0" }, { capabilities: { tools: {} } });
  ...
  const tools: Tool[] = [
    ...
    },
    {
      ...
      description: "Blocking convenience wrapper: start watch and wait until all filtered checks settle or timeout (default 600s). Emits progress notifications if supported.",
      inputSchema: {
        type: "object",
        properties: {
          owner: { type: "string" },
      ...
      switch (name) {
        case "start_pr_watch": {
          ...
          const handle = await wm.startWatch({
            ...
            ...(parsed.timeout_seconds !== undefined ? { timeoutMs: parsed.timeout_seconds * 1000 } : {}),
          });
        ...
        case "await_pr_actions": {
          ...
          const notify = (evt: { name: string; status: string; conclusion: string | null }) => {
            try {
              const srv = server as unknown as { notification: (n: unknown) => Promise<void> };
              if (srv.notification) {
                void srv.notification({
                  method: "notifications/progress",
                  params: { progress: 0, total: 1, message: `${evt.name}:${evt.status}:${evt.conclusion ?? ""}` },
                });
              }
            } catch {
              // ignore
...
export async function startStdio(opts?: { bus?: EventBus; watchManager?: WatchManager }): Promise<void> {
