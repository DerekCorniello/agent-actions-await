...
export class WatchManager {
    ...
    constructor(
        bus: EventBus,
        deps?: {
            getPrHeadShaFn?: typeof getPrHeadSha;
            getChecksFn?: typeof getChecksForShaWithGrace;
        },
    ...
    async startWatch(args: {
        ...
        const onEvent = (evt: NormalizedEvent) => {
            ...
            if (
                ...
                (evt.status === "closed" || evt.status === "merged")
            ) {
    ...
    getStatus(handle: string): WatchStatus | { error: string; hint?: string } {
        ...
        if (!e)
            return {
                ...
                hint: "watch already completed and GC'd or unknown handle",
            };
        // If settled and already consumed once, GC on this read (Q37 keep briefly until first read)
        if (
            (e.status.state === "completed" || e.status.state === "timed_out") &&
            e.consumedAfterSettled
        ) {
